...
def exponent_mechanism(mute_info_list, n_dataset, epsilon, d):
    sensitivity = 2. / n_dataset * np.log2(float((n_dataset + 1)) / 2) + float(n_dataset - 1) / n_dataset * np.log2(
        float(n_dataset + 1) / (n_dataset - 1))
...
def generate_N(d, theta, epsilon):  # d:属性个数
    ...
    print(k)
    # print k
    # A_set = [8, 10, 14, 1, 6, 13, 5, 7, 4, 9, 11, 12, 3, 2]  # 节点进入N顺序
    A_set = [7, 9, 13, 0, 5, 12, 4, 6, 3, 8, 10, 11, 2, 1]  # 节点进入N顺序
    Ax_Pi_distrs = []
    Pi_distrs = [[]]
    Pi_set = [[]]
    ...
    for i in range(len(A_set)):
        ...
        # print(mute_info_list)
    # Invalid argument
    sio.savemat('Ax_Pi_distribution-el.mat', mdict={'Ax_Pi_distribution': Ax_Pi_distrs})
    sio.savemat('P_set-el.mat', mdict={'P_set': Pi_set})
    sio.savemat('Pi_distribution-el.mat', mdict={'p_distribution': Pi_distrs})
    # sio.savemat('Index_mutual_info.mat', mdict={'D': index_mute_info_list})
    sio.savemat('network—el.mat', mdict={'N': N})
    # print(N)
    # print('Now print the Ax_Pi_distrs:')
    # with open('Ax-P-distribution.txt', 'w') as file:
    # 将结果转换为字符串格式后写入文件
    #    file.write(str(Ax_Pi_distrs))
    # print('Now print the Pi_distrs:')
    # with open('P-distribution.txt', 'w') as file:
    # 将结果转换为字符串格式后写入文件
    #    file.write(str(Pi_distrs))
    # print('Now print the Pi_set:')
    # with open('Pi-set.txt', 'w') as file:
    # 将结果转换为字符串格式后写入文件
    #    file.write(str(Pi_set))
